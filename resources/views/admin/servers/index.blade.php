<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                    @foreach($servers as $s)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                            ...
                            <td class="px-4 py-3 text-sm">{{ $s->display_name }} <span class="text-slate-400">({{ $s->type === 'getatext' ? 'primary-us' : ($s->type === 'multi_country' ? 'multi-country' : ($s->type === 'fivesim' ? 'worldwide' : 'legacy')) }})</span></td>
                            <td class="px-4 py-3">
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400">When a server is <strong>Active</strong>, customers can rent numbers from that slot. Match each row to the correct upstream base URL and API credentials.</p>
    </div>
</x-app-layout>
