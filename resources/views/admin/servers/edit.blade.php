        ...
        <form action="{{ route('admin.servers.update', $server) }}" method="POST" class="bg-white/80 dark:bg-slate-900/80 backdrop-blur rounded-2xl border border-slate-200 dark:border-slate-800 shadow-glass p-6 space-y-4">
            ...
            <div>
                ...
                <select name="type" class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 shadow-sm focus:border-mint-500 focus:ring-mint-500">
                    <option value="getatext" {{ old('type', $server->type) === 'getatext' ? 'selected' : '' }}>Slot A — US numbers (customer: Server 1)</option>
                    <option value="multi_country" {{ old('type', $server->type) === 'multi_country' ? 'selected' : '' }}>Slot B — multi-country (customer: Server 2)</option>
                    <option value="fivesim" {{ old('type', $server->type) === 'fivesim' ? 'selected' : '' }}>Slot C — worldwide (customer: Server 3)</option>
                    <option value="smsconfirmed" {{ old('type', $server->type) === 'smsconfirmed' ? 'selected' : '' }}>Legacy (disabled by default)</option>
                </select>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Slot A: API key header <code class="text-xs">Auth</code>. Slot C: <code class="text-xs">Authorization: Bearer …</code>. Match base URL to your upstream dashboard.</p>
            </div>
            <div>
